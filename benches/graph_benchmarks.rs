...
fn create_linear_chain(length: usize) -> GraphDescription {
    ...
    for i in 0..length {
        ...
        let mut node = NodeInstance::new(
            &node_id,
            "math.add",
            Position::new(100.0 * (i + 1) as f64, 0.0),
        );
...
fn create_wide_graph(width: usize) -> GraphDescription {
    ...
    for i in 0..width - 1 {
        ...
        let mut node = NodeInstance::new(
            &node_id,
            "math.multiply",
            Position::new(i as f64 * 100.0 + 50.0, 200.0),
        );
    ...
    let mut final_node = NodeInstance::new(
        "final_add",
        "math.add",
        Position::new(width as f64 * 50.0, 400.0),
    );
    final_node.inputs.push(PinInstance::new(
        "a",
        Pin::new("a", "A", DataType::typed("f64"), PinType::Input),
    ));
    final_node.inputs.push(PinInstance::new(
    ...
    final_node.outputs.push(PinInstance::new(
        "result",
        Pin::new("result", "Result", DataType::typed("f64"), PinType::Output),
    ));
    ...
    fn add_tree_level(
        ...
        *counter += 1;
        ...
        if current_depth == 0 {
            // Leaf node - constant
            let mut node =
                NodeInstance::new(&node_id, "math.constant", Position::new(x_offset, y_pos));
            ...
            let right_child = add_tree_level(
                graph,
                depth,
                current_depth - 1,
                &node_id,
...
fn create_control_flow_graph(num_branches: usize) -> GraphDescription {
    ...
    event.outputs.push(PinInstance::new(
        "exec",
        Pin::new("exec", "Exec", DataType::Exec, PinType::Output),
    ));
    ...
    for i in 0..num_branches {
        ...
        let mut branch = NodeInstance::new(
            &branch_id,
            "branch",
            Position::new(200.0 * (i + 1) as f64, 0.0),
        );

        branch.inputs.push(PinInstance::new(
            "exec",
            Pin::new("exec", "Exec", DataType::Exec, PinType::Input),
        ));
        branch.inputs.push(PinInstance::new(
            "condition",
            Pin::new(
        ...
        branch.outputs.push(PinInstance::new(
            "true",
            Pin::new("true", "True", DataType::Exec, PinType::Output),
        ));
        branch.outputs.push(PinInstance::new(
            "false",
            Pin::new("false", "False", DataType::Exec, PinType::Output),
        ));
        ...
        let mut print_true = NodeInstance::new(
            &print_true_id,
            "print",
            Position::new(200.0 * (i + 1) as f64, -150.0),
        );
        print_true.inputs.push(PinInstance::new(
            "exec",
            Pin::new("exec", "Exec", DataType::Exec, PinType::Input),
        ));
        print_true.inputs.push(PinInstance::new(
            "value",
            Pin::new("value", "Value", DataType::typed("String"), PinType::Input),
        ));
        print_true.outputs.push(PinInstance::new(
            "then",
            Pin::new("then", "Then", DataType::Exec, PinType::Output),
        ));
        ...
        let mut print_false = NodeInstance::new(
            &print_false_id,
            "print",
            Position::new(200.0 * (i + 1) as f64, 150.0),
        );
        print_false.inputs.push(PinInstance::new(
            "exec",
            Pin::new("exec", "Exec", DataType::Exec, PinType::Input),
        ));
        print_false.inputs.push(PinInstance::new(
        ...
        print_false.outputs.push(PinInstance::new(
            "then",
            Pin::new("then", "Then", DataType::Exec, PinType::Output),
        ));
        print_false.set_typed_property(
            "value",
            PropertyValue::String(format!("False branch {}", i)),
        );
    ...
    for row in 0..scale {
        for col in 0..scale {
            ...
            let mut node = NodeInstance::new(
                &node_id,
                "math.multiply",
                Position::new(col as f64 * 150.0, row as f64 * 150.0),
            );
...
fn bench_linear_chain(c: &mut Criterion) {
    ...
    for size in [10, 50, 100, 500, 1000].iter() {
        ...
        group.bench_with_input(BenchmarkId::new("sequential", size), size, |b, &_size| {
            b.iter(|| {
                let data_resolver =
                    DataResolver::build(black_box(&graph), black_box(&provider)).unwrap();
        ...
        group.bench_with_input(BenchmarkId::new("parallel", size), size, |b, &_size| {
            b.iter(|| {
                let data_resolver =
                    DataResolver::build_parallel(black_box(&graph), black_box(&provider)).unwrap();
                black_box(data_resolver);
            });
        });
    }
...
fn bench_monster_graph(c: &mut Criterion) {
    ...
    for scale in [10, 20, 30, 40, 50].iter() {
        ...
        group.bench_with_input(
            BenchmarkId::new("sequential", scale),
            scale,
            |b, &_scale| {
                b.iter(|| {
                    let data_resolver =
